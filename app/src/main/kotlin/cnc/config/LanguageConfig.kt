...
object PrintScriptTokenDefProvider : TokenDefinitionProvider {
  val definitions =
    mapOf<TokenType, List<TokenDefinition>>(
...
val terminators: List<TokenDefinition> =
  listOf(
...
val expressionBuilder =
  ExpressionBuilder(
    mapOf(
      PrintScriptTokenDefProvider.getDefinition("number_exp") to
        { token: Token -> NumberLiteral(token.text.toDouble()) },
      PrintScriptTokenDefProvider.getDefinition("string_exp") to
        { token: Token -> StringLiteral(token.text.removeSurrounding("\"")) },
